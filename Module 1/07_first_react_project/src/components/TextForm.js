    ...
    return (
        <>
            <div className='container'>
                ...
                <button disabled={text.length === 0} className="btn btn-primary mx-2" onClick={boldText}>Bold Text</button>
                <button disabled={text.length === 0} className="btn btn-primary mx-2" onClick={clearTheText}>Clear text</button>
                <button disabled={text.length === 0} className="btn btn-primary mx-2" onClick={changeColor}>Change Color</button>
            </div>
            ...
            <div className="container my-3">
                ...
                <ul className={`text-${props.mode === 'light' ? 'dark' : 'light'}`}>
                    ...
                    <li>No. of words: {text.split(/\s+/).filter((element) => element !== '').length}</li>
